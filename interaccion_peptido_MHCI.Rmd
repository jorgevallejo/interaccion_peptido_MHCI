...
```{r configura semilla aleatorizacion}
set.seed(1234567)
```
## Primer modelo
...
Para evaluar el modelo utilizamos el set de datos de prueba que habíamos especificado con anterioridad.

```{r evaluate model one performance}
model_results <- predict(model, test_dataframe)
model_results <- max.col(model_results)
model_results <- factor(one_column_model_results, labels = c("NB", "SB"))
...
model2 <- neuralnet(label ~ ., 
                   ...
                   linear.output = FALSE,
                   hidden = 1)
save(model, file = "./intermediateData/model2.RData")
}
```
