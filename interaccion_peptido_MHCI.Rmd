...
## Modelo de tres nodos ocultos entrenado por validación cruzada de 5 particiones
...
predicted.ann.caret <- predict(model.ann.caret,
                               newdata=test_dataframe)
# confusion matrix for caret ANN
# Set SB as the positive
performance.ann.caret <- confusionMatrix(predicted.ann.caret,
                test_dataframe$label,
                positive = "SB")
```
...
# Recupera los parametros de interes
five_fold_evaluation <- performance.ann.caret$byClass[c(1, 2, 5)]
# Create data frame
...
XXX Repetir la validación cruzada de forma más correcta. Sigue el modelo del libro de Lantz XXX Por otro lado, el enunciado del ejercicio sugiere que sí tengo que usar la funcion caret::train(). Quizá debería dar sólo el dato de accuracy y olvidar especificidad y precisión? XXX

El método de validación cruzada es un método de validación que se usa para evitar entrenar o testear los modelos con datos no representativos. En este caso, los resultados de la validación cruzada sobre el modelo nos dan **el mismo valor de sensibilidad que los dos modelos anteriores, pero peor especificidad y precisión**.
...
Sin embargo, no he usado el métode de validación en su forma correcta, ya que he apartado un subset de datos de entrenamiento para reproducir lo que he hecho con los dos modelos anteriores. El resultado de ello ha sido que los modelos producidos para validación cruzada se han calculado a partir de menos valores de entrenamiento. Quizá esa sea la razón de la menor especificidad y precisión.
...
# Best tuning result is with C = 1.00
# Next time you need to run it, fix it with tuneGrid
```

```{r evaluate svm_caret performance}
# Prediction for test values
predicted_svm_caret <- predict(svm_caret,
                               newdata=test_dataframe)
# confusion matrix for caret ANN
# Set SB as the positive
performance_svm_caret <- confusionMatrix(predicted_svm_caret,
                test_dataframe$label,
                positive = "SB")
```
...
# Recupera los parametros de interes
five_fold_svm_evaluation <- performance_svm_caret$byClass[c(1, 2, 5)]
# Create data frame
...
**El modelo gaussiano por validación cruzada** podría ser la mejor solución (para este tipo de datos) comparando los tres modelos construidos mediante SVM, ya que **combina la sensibilidad del modelo gaussiano con casi los mismos niveles de especificidad y precisión del modelo lineal**.
...
Vemos que no hemos conseguido una solución perfecta. Como comentábamos en apartados anteriores, la elección del modelo a elegir depederá de si preferimos **sensibilidad**, para lo cual elegiríamos el **modelo gaussiano por validación cruzada**; o si necesitamos que **especificidad y precisión** sean lo más altas posible, por lo que nos decantaríamos por el modelo de **red neural artificial con tres nodos en la capa oculta**.
